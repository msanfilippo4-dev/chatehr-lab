...
import { quizCsv } from "@/lib/server/quiz-export";
...
export async function GET(request: Request) {
  try {
    ...
    const scope = queryParam(request, "scope");
    if (scope.startsWith("quiz")) return quizCsv(admin, instructor.email, scope, { includeTest, week: queryParam(request, "week") });
