...
export function QueryStudio({ state, setState, audit }: { state: EHRState; setState: SetEHRState; audit: Audit }) {
  ...
  return <div className="grid two-one">
    ...
    <div className="stack"><Panel title="Coverage distribution" subtitle="A first stratification, not an equity conclusion"><div className="bar-chart">{coverage.map((row) => <div key={row.insurance}><span>{row.insurance}</span><div><i style={{ width: `${displayed.length ? (row.count / displayed.length) * 100 : 0}%` }} /></div><b>{row.count}</b></div>)}</div></Panel><Panel title="Validation checklist"><ul className="checklist"><li>Confirm the denominator and time window.</li><li>Inspect units, status, and missing values.</li><li>Test a sample against the source chart.</li><li>Separate measurement from causal claims.</li><li>Record the exact cohort definition.</li></ul><label>Validation note<textarea rows={5} value={validationNote} onChange={(event) => { setValidationNote(event.target.value); setValidationSaved(false); }} placeholder="State the denominator, patient-level check, data-quality limitation, and interpretation boundary." /></label><button className="primary" onClick={() => { if (latestIds.length === 0 || validationNote.trim().length < 80) return alert("Run a query and enter a validation note of at least 80 characters."); audit("Validate population query", `${queryName}: ${displayed.length} patient(s). ${validationNote.trim()}`); setValidationSaved(true); }}>Record validation</button>{validationSaved && <p className="form-message success">Validation evidence recorded.</p>}</Panel><Panel title="Saved query runs" subtitle="Reproducible evidence in this workspace"><div className="run-list">{state.queryRuns.length ? state.queryRuns.slice(0, 5).map((run) => <p key={run.id}><strong>{run.name}</strong><span>{run.rowCount} rows · {run.executedAt}</span></p>) : <p className="empty">No queries have been run.</p>}</div></Panel></div>
  </div>;
}
...
export function ImplementationReadiness({ state, setState, audit }: { state: EHRState; setState: SetEHRState; audit: Audit }) {
  ...
  return <div className="stack"><Panel title="Implementation readiness board" subtitle="Evidence-based preparation for a simulated barcode medication administration go-live"><div className="readiness-summary"><div><strong>{ready}/{state.implementation.length}</strong><span>domains ready</span></div><div><strong>{state.implementation.filter((item) => item.risk === "High" && item.status !== "Ready").length}</strong><span>open high-risk domains</span></div><div><strong>{state.implementation.filter((item) => item.status === "Blocked").length}</strong><span>blocked decisions</span></div></div><div className="inline-alert warn"><strong>Readiness is evidence, not optimism.</strong><p>A go-live decision should identify unresolved risk, accountable owners, mitigations, and measurable exit criteria.</p></div></Panel><div className="readiness-grid">{state.implementation.map((item) => <article className="readiness-card" key={item.id}><header><span>{item.domain}</span><Status tone={item.status === "Ready" ? "good" : item.status === "Blocked" ? "danger" : "warn"}>{item.status}</Status></header><h3>{item.requirement}</h3><dl><div><dt>Owner</dt><dd>{item.owner}</dd></div><div><dt>Evidence</dt><dd>{item.evidence}</dd></div><div><dt>Risk</dt><dd>{item.risk}</dd></div></dl><label>Readiness decision<select aria-label={`${item.domain} readiness status`} value={item.status} onChange={(event) => { update(item, event.target.value as ImplementationCheckpoint["status"]); setSaved(false); }}><option>Not started</option><option>In progress</option><option>Ready</option><option>Blocked</option></select></label></article>)}</div><Panel title="Release recommendation" subtitle="Record accountable conditions before go-live"><div className="decision-box"><label>Decision<select value={recommendation} onChange={(event) => { setRecommendation(event.target.value); setSaved(false); }}><option>No go</option><option>Conditional go</option><option>Go</option></select></label><label>Safeguards and evidence<textarea rows={6} value={recommendationNote} onChange={(event) => { setRecommendationNote(event.target.value); setSaved(false); }} placeholder="Name an accountable owner, release condition, outcome measure, balancing measure, and rollback trigger." /></label><button className="primary" onClick={() => { if (recommendationNote.trim().length < 120) return alert("Enter at least 120 characters describing safeguards and evidence."); audit("Record go-live recommendation", `${recommendation}. ${recommendationNote.trim()}`); setSaved(true); }}>Record recommendation</button>{saved && <p className="form-message success">Go-live recommendation recorded in the audit trail.</p>}</div></Panel></div>;
}
